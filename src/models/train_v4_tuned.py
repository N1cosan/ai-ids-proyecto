...
def train_model(X_train, y_train) -> RandomForestClassifier:
    model = RandomForestClassifier(
        n_estimators=200,
        max_depth=15,               # antes 20, reduce sobreajuste
        min_samples_leaf=3,         # antes 1 (default), evita hojas ultra-especificas
        class_weight="balanced_subsample",  # antes "balanced"
        n_jobs=-1,
...
if __name__ == "__main__":
    ...
    joblib.dump(model, MODEL_DIR / "rf_model_v4_tuned.joblib")
    joblib.dump(label_encoder, MODEL_DIR / "label_encoder_v4_tuned.joblib")
    joblib.dump(list(X.columns), MODEL_DIR / "feature_columns_v4_tuned.joblib")
